...
export const CreateCategoryModal = () => {
  ...
  const { addCategory } = useBoardMutations(boardId || "");

  const formSchema = z.object({
    name: z
      .string()
      .min(1, {
        message: t.modals.createCategory.nameRequired,
      })
      .max(50, { message: t.modals.createCategory.nameTooLong }),
  });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      addCategory({ ...response.data, channels: [] });
